...
// ==================== DEFAULT STATE ====================
const DEFAULT_GOALS = [
  { id: 'focus',    name: 'Stay Focused',     icon: '🎯' },
  ...
  { id: 'admin',    name: 'Admin & Chores',   icon: '🗂️' }
];
...
const LEVEL_TITLES = [
  ...
  { min: 2,  name: 'Spark',              icon: '✨' },
  { min: 3,  name: 'Tomato Picker',      icon: '🍅' },
  { min: 5,  name: 'Focus Forager',      icon: '🔥' },
  { min: 8,  name: 'Deep Worker',        icon: '⚡' },
  { min: 12, name: 'Brain on Fire',      icon: '🚀' },
  { min: 17, name: 'Hyperfocus Knight',  icon: '⚔️' },
  { min: 23, name: 'Tomato Sage',        icon: '🏆' },
  { min: 35, name: 'Dopamine Lord',      icon: '👑' },
  { min: 50, name: 'Dopamodoro Legend',  icon: '⭐' }
];
...
// ==================== ACHIEVEMENTS ====================
const ACHIEVEMENTS = [
  { id: 'first',    title: 'First Tomato',   desc: 'Finish your first session',  icon: '🍅', max: 1,   get: s => Math.min(s.totalTomatoes, 1) },
  { id: 'level10',  title: 'Level 10',        desc: 'Reach level 10',             icon: '⭐', max: 10,  get: s => Math.min(s.level, 10) },
  { id: 'century',  title: 'Century Club',    desc: 'Complete 100 sessions',      icon: '💯', max: 100, get: s => Math.min(s.totalTomatoes, 100) },
  { id: 'level20',  title: 'Level 20',        desc: 'Reach level 20',             icon: '🏆', max: 20,  get: s => Math.min(s.level, 20) },
  { id: 'week7',    title: 'Week Warrior',    desc: '7-day focus streak',         icon: '🔥', max: 7,   get: s => Math.min(s.streak, 7) },
  { id: 'legend250',title: 'Focus Legend',    desc: 'Complete 250 sessions',      icon: '👑', max: 250, get: s => Math.min(s.totalTomatoes, 250) },
  { id: 'streak30', title: 'Month of Fire',   desc: '30-day streak',              icon: '🚀', max: 30,  get: s => Math.min(s.streak, 30) },
  { id: 'toms500',  title: 'Harvest Master',  desc: '500 sessions completed',     icon: '🌟', max: 500, get: s => Math.min(s.totalTomatoes, 500) },
];
...
function updateLevelChip() {
  ...
  document.getElementById('levelChipText').textContent = `${t.icon} ${t.name}`;
}
...
function renderTaskPills() {
  ...
  if (!tasks.length) {
    ...
    tasks.forEach((t, i) => {
      ...
      const clientChip = cl
        ? `<button class="tf-client" style="--cc:${cl.color}" title="${escapeHtml(cl.name)}">${cl.type === 'client' ? '💼' : '🏠'}</button>`
        : `<button class="tf-client tf-client-none" title="Tag client / personal">💼</button>`;
      el.innerHTML = `<span class="tf-dot">📌</span><span class="tf-name">${escapeHtml(ctName(t))}</span>${clientChip}${tag}<button class="tf-x" title="Remove">×</button>`;
...
function openClientPickerCore(currentId, onPick) {
  ...
  const row = c => `<button class="fp-item ${c.id === currentId ? 'active' : ''}" data-cid="${c.id}"><span class="fp-dot" style="background:${c.color}"></span><span class="fp-name">${c.type === 'client' ? '💼' : '🏠'} ${escapeHtml(c.name)}</span>${c.id === currentId ? '<span class="fp-check">✓</span>' : ''}</button>`;
  ...
  if (clientsC.length) html += `<div class="fk-group">💼 Clients</div>` + clientsC.map(row).join('');
  if (personalC.length) html += `<div class="fk-group">🏠 Personal</div>` + personalC.map(row).join('');
...
function renderNorthStar() {
  ...
  if (state.northStarGoal) {
    root.innerHTML = `<div class="ns-card-set">
      <div class="ns-eyebrow">North Star ✨</div>
    ...
    root.innerHTML = `<button class="ns-cta" id="nsCtaBtn">✨ Set your big-picture goal</button>`;
...
function openCompletedHistory() {
  ...
  if (!done.length) {
    body.innerHTML = `<div class="todo-empty"><div class="todo-empty-icon">✅</div><div class="todo-empty-title">No completed tasks yet</div><div class="todo-empty-sub">Finish a task and it'll be recorded here by day.</div></div>`;
...
function renderTodoCard(t) {
  ...
  return `<div class="todo-item pri-${pri} ${t.done ? 'is-done' : ''}" data-id="${t.id}">
    ...
    <div class="todo-actions">
      ...
      ${!t.done ? `<button class="todo-pin ${pinned ? 'is-pinned' : ''}">📌</button>` : ''}
...
function renderHistory() {
  ...
  if (hist.length === 0) {
    list.innerHTML = `<div class="todo-empty"><div class="todo-empty-icon">📓</div><div class="todo-empty-title">Your journal is empty</div><div class="todo-empty-sub">Finish a focus session and it'll show up here.</div></div>`;
    return;
  }
  const byDate = {};
  ...
  dates.forEach(date => {
    ...
    if (ref && (ref.text || ref.rating)) {
      const emoji = ['','😔','😐','😊','🤩'][ref.rating || 0] || '';
      html += `<div class="day-reflect-preview" data-day="${date}">${emoji ? `<span class="drp-emoji">${emoji}</span>` : ''}<span class="drp-text">${escapeHtml((ref.text || '').slice(0, 80))}${(ref.text || '').length > 80 ? '…' : ''}</span></div>`;
    }
    entries.slice().reverse().forEach(e => {
      ...
      if (e.type === 'work' || e.type === 'workManual') {
        html += `<div class="history-entry" data-eid="${e.id}">
          <span class="he-icon">${escapeHtml(e.goalIcon || '🍅')}</span>
          <div class="he-body">
...
function updateGoalSelectorBtn() {
  ...
  if (!btn || !txt) return;
  const goal = (state.goals || []).find(g => g.id === state.activeGoal);
  txt.textContent = goal ? `${goal.icon} ${goal.name}` : '🎯 Set category';
...
function openGoalSelector() {
  ...
  let html = goals.map(g => `
    <button class="goal-option ${state.activeGoal === g.id ? 'active' : ''}" data-gid="${g.id}">
      <span class="goal-opt-icon">${g.icon}</span>
...
function renderAchievementsSheet() {
  ...
  let html = `<div class="ach-hero">
    <div class="ach-hero-icon">${t.icon}</div>
...
function openJournalEntry(id) {
  ...
  document.getElementById('jeHeader').textContent = isWork ? '🍅 Focus Session' : '🧘 Break';
...
function renderEntrySplit(entry) {
  ...
  const rows = meta.map((m, i) => {
    ...
    const tag = cl
      ? `<button class="jes-client" style="--cc:${cl.color}">${cl.type === 'client' ? '💼' : '🏠'} ${escapeHtml(cl.name)}</button>`
  ...
  wrap.querySelectorAll('.jes-client').forEach(btn => btn.addEventListener('click', () => {
    ...
    openClientPickerCore(row.dataset.cid || null, cid => {
      ...
      if (cl) { btn.style.setProperty('--cc', cl.color); btn.textContent = (cl.type === 'client' ? '💼 ' : '🏠 ') + cl.name; }
...
function renderDeadlineStrip() {
  ...
  el.innerHTML = `
    <span class="ds-icon">${top.d.cls === 'overdue' ? '⚠️' : '⏰'}</span>
    <div class="ds-body">
...
function coachTips(ins) {
  ...
  if (overdue.length) tips.push({ icon: '⚠️', text: `${overdue.length} task${overdue.length > 1 ? 's' : ''} overdue — start with just 15 minutes on one.` });
  if (ins.todaySessions === 0) tips.push({ icon: '🌅', text: "You haven't focused yet today. One short session is enough to start." });
  if (state.streak >= 3 && ins.todaySessions === 0) tips.push({ icon: '🔥', text: `Your ${state.streak}-day streak is at risk — protect it with one session today.` });
  if (ins.peakHour != null && ins.peakCount >= 3) tips.push({ icon: '⏰', text: `You focus best around ${fmtHour(ins.peakHour)}. Block that hour for your hardest task.` });
  if (ins.lastWeek > 0 && ins.thisWeek >= ins.lastWeek) tips.push({ icon: '📈', text: `Matching or beating last week (${ins.thisWeek} vs ${ins.lastWeek}). Keep the wave going.` });
  if (ins.lastWeek > 0 && ins.thisWeek < ins.lastWeek) tips.push({ icon: '💪', text: 'A little behind last week — a couple of sessions today closes the gap.' });
  if (!tips.length) tips.push({ icon: '✨', text: 'Consistency beats intensity. Show up again tomorrow and the score climbs.' });
...
function renderCoachDashboard() {
  ...
  document.getElementById('csMetrics').innerHTML = `
    ...
    <div class="cs-metric"><div class="cs-metric-val">${state.streak} 🔥</div><div class="cs-metric-lbl">Day streak</div></div>
  ...
  document.getElementById('csTips').innerHTML = coachTips(ins).map(t => `
    <div class="cs-tip"><span class="cs-tip-icon">${t.icon}</span><span>${t.text}</span></div>`).join('');
...
function renderClientHours() {
  ...
  if (!entries.length) {
    el.innerHTML = `<div class="todo-empty-sub" style="text-align:center;padding:10px;">Tag tasks with a client/personal (💼/🏠) and split session time in the Journal to see hours here.</div>`;
    return;
  }
  ...
  el.innerHTML = entries.map(o => `<div class="pb-row"><span class="pb-dot" style="background:${o.c.color}"></span><span class="pb-name">${o.c.type === 'client' ? '💼' : '🏠'} ${escapeHtml(o.c.name)}</span><span class="pb-time">${fmtH(o.min)}</span></div>`).join('') +
...
function pickHomeNudge() {
  ...
  const candidates = [];
  if (t >= 1) candidates.push({ key: 'n_journal', icon: '📖', text: 'Your sessions are saved in the Journal — take a look.', view: 'journal' });
  if (t >= 3) candidates.push({ key: 'n_heatmap', icon: '🔥', text: "You're building a habit — check your Focus Map.", view: 'journal', heatmap: true });
  if (t >= 5) candidates.push({ key: 'n_coach', icon: '🧠', text: 'Enough data to spot patterns — meet your Insights.', view: 'coach' });
...
function renderCoachNudge() {
  ...
  el.classList.remove('hidden');
  el.innerHTML = `<span class="cn-icon">${n.icon}</span><span class="cn-text">${n.text}</span><button class="cn-x" title="Dismiss">×</button>`;
...
function renderTomorrowLaunchpad() {
  ...
  el.classList.remove('hidden');
  el.innerHTML = `
    <div class="lp-eyebrow">🌅 Today's focus — set last night</div>
...
function renderWrapTrigger() {
  ...
  if (wrapped && wrapped.wrappedAt) {
    ...
    el.innerHTML = `<span class="wt-icon">🌙</span><span class="wt-text">Day wrapped — tap to review</span>`;
  } else {
    el.classList.remove('is-done');
    el.innerHTML = `<span class="wt-icon">🌙</span><span class="wt-text">Wrap up today — log a win &amp; set tomorrow's focus</span><span class="wt-go">›</span>`;
  }
...
function renderWinVault() {
  ...
  el.innerHTML = wins.map(([d, w]) =>
    `<div class="win-item"><div class="win-date">${dayLabel(d)}</div><div class="win-text">🏆 ${escapeHtml(w.win)}</div></div>`
  ).join('');
}
...
function renderReminderPill() {
  ...
  el.classList.remove('hidden');
  el.innerHTML = `<span class="rp-icon">⏰</span>
...
function onbGo(step) {
  document.querySelectorAll('.onb-step').forEach(s => s.classList.toggle('active', s.dataset.step === String(step)));
}
...
function wireOnboarding() {
  document.querySelectorAll('#onbOverlay [data-go]').forEach(b => b.addEventListener('click', () => onbGo(b.dataset.go)));
  document.querySelectorAll('#onbOverlay [data-skip]').forEach(b => b.addEventListener('click', finishOnboarding));
  document.getElementById('onbCreateFolder')?.addEventListener('click', () => {
    const name = (document.getElementById('onbFolder').value || '').trim();
    if (name) addTodoGroupSilent(name, gid => { onbFolderId = gid; });
    onbGo(3);
  });
  document.getElementById('onbFinish')?.addEventListener('click', () => {
    const txt = (document.getElementById('onbTask').value || '').trim();
    if (txt) {
      state.todos.unshift({
        id: 'td' + Date.now() + Math.random().toString(36).slice(2, 6),
        text: txt, done: false, priority: 3, notes: [], subtasks: [],
        goalId: null, dueDate: null, groupId: onbFolderId || null,
        createdAt: new Date().toISOString(), completedAt: null
      });
      state.currentTasks.push({ name: txt, groupId: onbFolderId || null, clientId: null });
    }
    finishOnboarding();
    // Deliver the aha immediately: the button promised "start focusing", so start.
    switchView('timer');
    state.mode = 'work';
    state.sessionDuration = getTotalForMode('work');
    startTimer();
